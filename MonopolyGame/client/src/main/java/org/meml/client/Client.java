...
public class Client
{
    ...
    public void run() {
            ...
            if (currentInput != null && currentInput.length()>0) {
                ...
                if (currentMsg.getIdCreateResponseCount() > 0) {
                    ...
                    if (response.getStatus() == ServerClient.Status.SUCCESS) {
                        ...
                        logger.info(String.format("Create Id failed, reason:%s", response.getFailReason()));
                        out.println("Create Id failed, reason:" + response.getFailReason());
                    }
                }
            }
        } while (creatingId);
                ...
                case "create":
                    ...
                    if (currentMsg.getRoomCreateResponseCount() > 0) {
                        ...
                        if (response.getStatus() == ServerClient.Status.SUCCESS) {
                            ...
                            logger.info(String.format("Create Room failed, reason:%s", response.getFailReason()));
                            out.println("Create Room failed, reason:" + response.getFailReason());
                        }
                    }
                    break;
                case "join":
                    ...
                    if (currentMsg.getRoomJoinResponseCount() > 0) {
                        ...
                        if (response.getStatus() == ServerClient.Status.SUCCESS) {
                            ...
                            logger.info(String.format("Join Room failed, reason:%s", response.getFailReason()));
        ...
        if (currentMsg.getGameStartInformCount() > 0) {
            ...
            clientServerCommunicator.send(ClientServer.CSmsg.newBuilder().addGameStartAck(
                    ClientServer.GameStartAck.newBuilder()
                            .setPlayerName(playerName)).build());
        }
